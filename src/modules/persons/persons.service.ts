...
export class PersonsService {
  ...
  async createFullPerson(
    data: { name: string; cpf?: string; cnpj?: string; companyName?: string },
    ...
    try {
      ...
      if (cleanCpf) {
        await queryRunner.manager.save(PhysicalPersonEntity, {
          id: id,
          cpf: cleanCpf,
        });
      } else if (cleanCnpj) {
  ...
  async findIdByDocument(doc: {
    cpf?: string;
    cnpj?: string;
  }): Promise<number | null> {
    const cleanDoc = doc.cpf
      ? this.clearString(doc.cpf)
      : doc.cnpj
        ? this.clearString(doc.cnpj)
        : null;
