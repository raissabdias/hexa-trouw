...
export class ListTravelsUseCase {
    ...
    async execute(page: number = 1, limit: number = 10) {
        ...
        const { data, total } = await this.travelRepo.findAll(page, limit, companyId);

        const formattedTravels = await Promise.all(data.map(async (travel) => {
            ...
            const travelPoints = await Promise.all(
                Array.isArray(pointsRaw) ? pointsRaw.map(async (point: any) => {
            ...
            const originPoint = travelPoints.find(p => p.stopTypeId === 4) || travelPoints[0];
