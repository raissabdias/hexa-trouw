...
export class CreateLocationUseCase {
  ...
  async execute(input: CreateLocationInput, userCompanyId?: number) {
    ...
    if (personId) {
      ...
      if (existingLocation) {
        throw new ConflictException(
          'This person already has a registered location. Multiple locations per person are not allowed.',
        );
      }
    } else {
